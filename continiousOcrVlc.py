...
tempCor = {"XTL":879, "YTL": 401,"XBR":903, "YBR":421}  
...
# taking screen shot
    ...
    '''
    '''
    temp = reader.readtext(binary_temp, batch_size=5)
    bpm = reader.readtext(binary_bpm, batch_size=5)
    spo2 = reader.readtext(binary_spo2, batch_size=5)
    pr = reader.readtext(binary_pr, batch_size=5)
    resp = reader.readtext(binary_resp, batch_size=5)
    blood_pressure = reader.readtext(binary_blood_pressure, batch_size=5)
    mmHg = reader.readtext(binary_mmHg, batch_size=5)
    '''
    filter = np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]])
    filter_2 = np.array([[3, -2, -3], [-4, 8, -6], [5, -1, -0]])
    # Applying cv2.filter2D function on our Logo image
    sharpen_img_2=cv2.filter2D(cropped_temp,-2,filter)
    ...
    scale_up_x = 1.9
    scale_up_y = 1.9
    scaled_up_temp = cv2.resize(sharpen_img_2 , None, fx= scale_up_x, fy= scale_up_y, interpolation= cv2.INTER_LINEAR)
    ...
    try:
        cv2.putText(frame, temp_print, (tempCor["XTL"],tempCor["YBR"] + 18), font, fontScale, color, thinckness, cv2.LINE_AA)
        ...
        cv2.putText(frame, resp_print, (respCor["XTL"], respCor["YBR"] + 25), font, fontScale, color, thinckness, cv2.LINE_AA)
    ...
    #webhook = DiscordWebhook(url=discordWebhookUrl, content= webhook_print) 
    #webhook.execute()
    #time.sleep(12) #discord will block if we upload within 17 seconds. So 12 Seconds Delay added with loop delay about 6 seconds
